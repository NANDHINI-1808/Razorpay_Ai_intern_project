...
export function Drawer({
  ...
  return createPortal(
    <div className="fixed inset-0 z-50 flex justify-end">
      <div
        className="absolute inset-0 bg-ink-900/40 backdrop-blur-[1px] transition-opacity"
        onClick={onClose}
        aria-hidden
      />
      <div
        ...
        aria-modal="true"
        aria-label={title}
        className="relative flex h-full w-full max-w-md flex-col bg-surface shadow-2xl animate-[slideIn_0.2s_ease-out]"
      >
        <div className="flex items-start justify-between border-b border-ink-100 px-6 py-5">
          ...
          <button
            onClick={onClose}
            className="rounded-lg p-1.5 text-ink-500 transition-colors hover:bg-canvas"
